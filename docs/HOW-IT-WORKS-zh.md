# 工作原理

本文档详细介绍讨论模式系统的内部架构和机制。

> 快速开始和安装说明请参见 [README](README-zh.md)。

---

## 概述

讨论模式采用 **双技能 + Hooks** 架构：

```
┌─────────────────────────────────────────────────────────────────┐
│                        AI 平台                                   │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │                         技能 (Skills)                       │ │
│  │  ┌─────────────────────┐  ┌─────────────────────────────┐  │ │
│  │  │ discuss-coordinator │  │      discuss-output         │  │ │
│  │  │  • 问题追踪         │  │  • 大纲渲染                 │  │ │
│  │  │  • 趋势分析         │  │  • 决策文档                 │  │ │
│  │  │  • 共识检测         │  │  • 文件管理                 │  │ │
│  │  └─────────────────────┘  └─────────────────────────────┘  │ │
│  └────────────────────────────────────────────────────────────┘ │
│                              │                                   │
│                              ▼                                   │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │                         钩子 (Hooks)                        │ │
│  │  ┌─────────────────────┐  ┌─────────────────────────────┐  │ │
│  │  │  track_file_edit    │  │   check_precipitation       │  │ │
│  │  │  (文件编辑时触发)   │  │   (对话结束时触发)          │  │ │
│  │  └─────────────────────┘  └─────────────────────────────┘  │ │
│  └────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
```

---

## 技能架构

### discuss-coordinator

**职责**：促进讨论流程、追踪问题、引导决策。

**功能**：
- 解析用户意图，识别讨论主题
- 追踪问题生命周期（🔵 当前焦点 → ⚪ 待定 → ✅ 已确认）
- 分析趋势，检测共识
- 判断决策何时可以沉淀

### discuss-output

**职责**：管理结构化输出和文档。

**功能**：
- 每轮后渲染和更新 `outline.md`
- 在 `decisions/` 目录创建决策文档
- 管理 `meta.yaml` 状态追踪
- 归档讨论和笔记

---

## Hooks 架构

Hooks 自动化不需要 AI 智能的"流程工作"：

### 设计原则

> **智能工作给 Agent，流程工作给 Hook**

| 工作类型 | 处理者 | 示例 |
|---------|--------|------|
| 智能 | AI 技能 | 理解问题、分析方案、识别共识 |
| 流程 | Python Hooks | 计数轮次、检查停滞、文件操作 |

### Hook 1: 文件编辑追踪

**触发时机**：AI 每次编辑文件后
- Claude Code: `PostToolUse` 事件 (matcher: `Edit|Write|MultiEdit`)
- Cursor: `afterFileEdit` 事件

**行为**：
1. 检测编辑的文件是否在讨论目录中
2. 在 `meta.yaml` 中设置 `pending_update: true`
3. 始终允许操作继续

### Hook 2: 沉淀检测

**触发时机**：AI 对话结束时
- Claude Code: `Stop` 事件
- Cursor: `stop` 事件

**行为**：
1. 扫描 `pending_update: true` 的讨论
2. 更新 `last_modified_run` 并清除 `pending_update`
3. 递增 `current_run` 计数器
4. 检查停滞文件（N 轮未更新）
5. 如果停滞，提醒用户更新大纲/决策

### 两阶段提醒逻辑

| 阶段 | 阈值 | 行为 |
|------|------|------|
| 建议 | `suggest_update_runs` (默认: 3) | 轻柔提醒 |
| 强制 | `force_update_runs` (默认: 10) | 强烈提醒，可能阻止 |

---

## 讨论目录结构

每个讨论创建一个结构化目录：

```
discuss/YYYY-MM-DD/topic-name/
├── outline.md          # 实时进度追踪
├── meta.yaml           # 元数据和轮次计数
├── decisions/          # 沉淀的决策
│   ├── D01-topic.md
│   ├── D02-topic.md
│   └── ...
└── notes/              # 参考资料
    └── ...
```

### outline.md

讨论状态的实时追踪：

```markdown
# 讨论：主题名称

## 🔵 当前焦点
- 当前正在讨论的问题

## ⚪ 待定
- [ ] Q1: 未回答的问题
- [x] ~~Q2: 已回答的问题~~ → 见决策

## ✅ 已确认
- D1: 第一个决策 → [文档](decisions/D01-xxx.md)
- D2: 第二个决策 → [文档](decisions/D02-xxx.md)

## ❌ 已否决
- (被否决的选项及理由)
```

### meta.yaml

由 Hooks 维护的状态追踪：

```yaml
# Hook 维护的字段
created_at: "2026-01-20T10:00:00+08:00"
current_run: 5

# 配置
config:
  suggest_update_runs: 3
  force_update_runs: 10

# 文件状态追踪
file_status:
  outline:
    last_modified_run: 4
    pending_update: false
  decisions:
    last_modified_run: 2
    pending_update: false
```

---

## 安装后的组件分布

### 全局（用户级别）

```
~/.config/DiscussForSpecs/
└── hooks/                    # Python hook 脚本
    ├── common/               # 共享工具
    ├── file-edit/            # 文件编辑追踪 hook
    └── stop/                 # 沉淀检测 hook

~/DiscussForSpecs/
└── logs/                     # Hook 执行日志
    └── discuss-hooks-YYYY-MM-DD.log
```

### 平台特定

**Claude Code**:
```
~/.claude/
├── skills/
│   ├── discuss-coordinator/  # 协调技能
│   └── discuss-output/       # 输出技能
└── settings.json             # Hooks 配置
```

**Cursor**:
```
~/.cursor/
├── skills/
│   ├── discuss-coordinator/  # 协调技能
│   └── discuss-output/       # 输出技能
└── hooks.json                # Hooks 配置
```

### 项目级别（使用 --target）

使用 `--target /path/to/project` 时：
```
/path/to/project/
└── .cursor/                  # 或 .claude/
    └── skills/
        ├── discuss-coordinator/
        └── discuss-output/
```

---

## 日志

Hooks 记录所有操作用于调试：

**位置**：`~/DiscussForSpecs/logs/discuss-hooks-YYYY-MM-DD.log`

**格式**：
```
2026-01-21 22:31:40 | INFO     | discuss-hooks | Hook Started: track_file_edit
2026-01-21 22:31:40 | DEBUG    | discuss-hooks | Input Data: {"file_path": "..."}
2026-01-21 22:31:40 | INFO     | discuss-hooks | Discussion detected: /path/to/discuss/topic
2026-01-21 22:31:40 | INFO     | discuss-hooks | Hook Ended: track_file_edit [SUCCESS]
```

---

## 平台支持

| 平台 | 技能 | Hooks | 状态 |
|------|------|-------|------|
| Claude Code | ✅ | ✅ | 就绪 |
| Cursor | ✅ | ✅ | 就绪 |
| VS Code Copilot | ⏳ | ⏳ | 计划中 |
| Windsurf | ⏳ | ⏳ | 计划中 |

---

## 相关链接

- [README](README-zh.md) - 快速开始和安装
- [AGENTS.md](AGENTS.md) - AI 代理指南
- [架构讨论](discuss/2026-01-17/skill-discuss-architecture-design/outline.md) - 设计决策

---

**最后更新**：2026-01-22
